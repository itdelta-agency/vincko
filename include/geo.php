...
class Geo {
    ...
    public function get_data($cookie = true) {
        ...
        if ($cookie && filter_input(INPUT_COOKIE, 'geo')) {
            return unserialize(filter_input(INPUT_COOKIE, 'geo'));
        }
        $data = $this->get_geobase_data();
        if (!empty($data)) {
            setcookie('geo', serialize($data), time() + 3600 * 24 * 7, '/'); //устанавливаем куки на неделю
        }
        return $data;
    }
