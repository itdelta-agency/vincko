...
<div class="wrap">
	...
	<div class="filter">
		<form action="/pakety-okhrannykh-uslug/packet-company.php" method="GET">
		<input type="hidden" name="packetID" value="<?=$_REQUEST['packetID']?>">
			...
			<div class="chop__flex">
				...
				<div class="chop__detail">
					...
					<div class="chop__detail-button">
						<a class="chop__detail-link" href="/pakety-okhrannykh-uslug/<?=$arResult["PACKET_CODE"]?>/?op=<?=$arItem["ID"]?><?if ($_REQUEST["eq"]):?>&eq=<?=intval($_REQUEST["eq"])?><?endif;?>">Выбрать</a>
					</div>
				</div>
			</div>
		</div>
